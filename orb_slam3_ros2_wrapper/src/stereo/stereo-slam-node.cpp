...
namespace ORB_SLAM3_Wrapper
{
        ...
        : Node("ORB_SLAM3_RGBD_ROS2")
    {
        ...
        this->declare_parameter("rgb_left_image_topic_name", rclcpp::ParameterValue("camera/left/image_raw"));
        this->declare_parameter("rgb_right_image_topic_name", rclcpp::ParameterValue("camera/right/image_raw"));
        this->declare_parameter("imu_topic_name", rclcpp::ParameterValue("imu"));

        // ROS Subscribers
        rgbLeftSub_ = std::make_shared<message_filters::Subscriber<sensor_msgs::msg::Image>>(this, this->get_parameter("rgb_left_image_topic_name").as_string());
        rgbRightSub_ = std::make_shared<message_filters::Subscriber<sensor_msgs::msg::Image>>(this, this->get_parameter("rgb_right_image_topic_name").as_string());
        syncApproximate_ = std::make_shared<message_filters::Synchronizer<approximate_sync_policy>>(approximate_sync_policy(10), *rgbLeftSub_, *rgbRightSub_);
    ...
    void StereoSlamNode::StereoCallback(const sensor_msgs::msg::Image::SharedPtr msgRgbLeft,
        const sensor_msgs::msg::Image::SharedPtr msgRgbRight)
    {
        Sophus::SE3f Tcw;
        if (interface_->trackStereo(msgRgbLeft, msgRgbRight, Tcw))
        {
            isTracked_ = true;
            ...
            if (publish_tf_)
            {
                ...
                if (!odometry_mode_)
                {
                    ...
                    tfMapRobot.header.stamp = msgRgbLeft->header.stamp;
                    tfMapRobot.header.frame_id = global_frame_;
                    tfMapRobot.child_frame_id = odom_frame_id_;
                    interface_->getDirectMapToRobotTF(msgRgbLeft->header, tfMapRobot);
            ...
            pose.header.stamp = msgRgbLeft->header.stamp;
